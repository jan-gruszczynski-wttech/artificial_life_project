...
Instance::Instance(string filename, bool parse_data_to_matrix) {
    ...
    ifstream f(filename);
    if (f.fail()) {
        cout << "Something went wrong while loading the file." << "\r\n";
        return;
    }
    fileName = filename;
    string line;
    ...
    for (int i = 0; i < file.size(); i++) {
        if (file[i] == "NODE_COORD_SECTION" || file[i] == "EDGE_WEIGHT_SECTION") {
            data_start = i;
            break;
        }
    }
    ...
    if (size < 0) {
        cout << "Bad instance size. File is corrupted and/or not supported." << "\r\n";
        return;
    }
    ...
    if (is_instance_data_in_matrix) {
        ...
        for (int i = 0; i < size; i++) matrix_copy[i] = new int[size];
    }
    ...
    if (edge_weight_type == "EXPLICIT") {
        ...
        if (!is_instance_data_in_matrix) {
            cout << "Explicit-type instances cannot be stored as coords." << "\r\n";
            size = -1;
            return;
        }
        ...
        if (edge_weight_type == "EUC_2D") {
            ...
            if (is_instance_data_in_matrix) {
                for (int j = 0; j < size; j++) {
                    for (int k = 0; k < size; k++) {
                        int distance = euc_2d_distance(j, k);
                        matrix[j][k] = distance;
                        matrix_copy[j][k] = distance;
                    }
                }
...
int Instance::edge_weight(int from, int to) {
    if (is_instance_data_in_matrix) return matrix_copy[from][to];

    switch (coords_weight_type) {
        case CoordsWeightType::EUC_2D: {
            return euc_2d_distance(from, to);
        }
        case CoordsWeightType::EUC_3D: {
            return euc_3d_distance(from, to);
        }
        case CoordsWeightType::MAX_2D: {
            return max_2d_distance(from, to);
        }
        case CoordsWeightType::MAX_3D: {
            return max_3d_distance(from, to);
        }
        case CoordsWeightType::MAN_2D: {
            return man_2d_distance(from, to);
        }
        case CoordsWeightType::MAN_3D: {
            return man_3d_distance(from, to);
        }
        case CoordsWeightType::CEIL_2D: {
            return ceil_2d_distance(from, to);
        }
        case CoordsWeightType::GEO: {
            return geo_distance(from, to);
        }
        case CoordsWeightType::ATT: {
            return att_distance(from, to);
        }
        case CoordsWeightType::COORDS_UNDEFINED: {
            return -1;
        }
        default:
            return -1;
